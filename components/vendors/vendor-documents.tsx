...
} from "@/app/(app)/vendors/document-actions";
import {
  MAX_DOCUMENT_BYTES,
  safeDocumentName,
...
import type { VendorDocument } from "@/lib/queries/vendors";

/**
 * Vendor attachments — brochures, price lists, certificates. Same uploader the
 * client record uses: the browser PUTs the file straight to Vercel Blob via the
 * shared /api/documents/upload token route, then registers the metadata through
 * `saveVendorDocument`. The bytes NEVER travel through a server action (Next
 * caps those bodies at 1 MB while this UI promises 25 MB).
 */

/** Vendor blobs nest under the shared `documents/` prefix the token route enforces. */
const VENDOR_DOCS_PREFIX = "documents/vendor-documents/";

/** File types a vendor attachment realistically is. */
const ACCEPT =
...
export function VendorDocuments({
  ...
  /** Admins upload + remove; everyone else reads and downloads. */
  canEdit: boolean;
}) {
  ...
  async function handleFile(file: File): Promise<void> {
    ...
    try {
      ...
      const blob = await upload(
        `${VENDOR_DOCS_PREFIX}${vendorId}/${safeDocumentName(file.name)}`,
        file,
        {
          access: "private",
      ...
      const res = await saveVendorDocument({
        vendorId,
        title: file.name,
  ...
  async function handleDelete(id: string, title: string): Promise<void> {
    ...
    setDeletingId(id);
    try {
      const res = await deleteVendorDocument(id);
      if (!res.ok) {
        fireToast({ message: res.error, type: "error" });
        return;
      }
    ...
    <div className="px-4 py-3.5">
      ...
      {documents.length === 0 ? (
        <p className="text-[13.5px] text-[#8a90a0]">
          {canEdit
            ? "Attach the vendor's brochure, price list or certificate — up to 25 MB per file."
