...
const ITEM_DOCS_PREFIX = "documents/item-documents/";
...
export function ItemDocuments({
  ...
  async function handleDelete(id: string): Promise<void> {
    ...
    setDeletingId(id);
    try {
      const res = await deleteItemDocument(id);
      if (!res.ok) {
        fireToast({ message: res.error });
        return;
      }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs text-gray-500">
          Attach drawings, specs, or inspection reports (max 25 MB each).
        </p>
        <label
      ...
      {documents.length === 0 ? (
        <p className="text-sm text-gray-500">No drawings or documents attached yet.</p>
      ) : (
