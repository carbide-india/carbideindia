...
- [x] **2.1 Persist notification dispatch attempts.**
  - **What:** New table `notification_dispatch_log(id, notification_id, channel, status, error, attempted_at)`. Each email/push send writes a row. Failures retry up to 3× with backoff via a cron route (`/api/cron/retry-notifications`).
  - **Why:** Sends are fire-and-forget; failures get a `console.error` and nothing more. People silently miss task assignments.
  - **Verify:** Kill the Resend key temporarily, trigger a task assignment, see 3 retry rows then a `failed` status.
...
- [x] **4.4 `.env.example`.**
  ...
  - **Outcome:** Created. Grouped by purpose (database, supabase, firebase, app, email, web-push, cron, observability, dev-only); calls out NEXT_PUBLIC_ vs server-only and the 5432-vs-6543 pooler-port rule; includes `SLOW_QUERY_MS` from Phase 0.1. Commit `5982ec2`.
